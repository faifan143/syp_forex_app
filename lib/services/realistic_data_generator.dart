...
class RealisticDataGenerator {
  ...
  static List<Map<String, double>> generateCandlestickData({
    ...
    if (symbol.contains('JPY')) {
      actualVolatility = volatility * 0.8; // JPY pairs are typically less volatile
    ...
    final List<double> sessionMultipliers = [0.8, 1.2, 0.6, 1.0]; // Different volatility per session
    
    for (int i = 0; i < count; i++) {
      // Determine market session (simplified)
      final int sessionIndex = (i ~/ (count ~/ 4)) % 4;
      final double sessionVolatility = actualVolatility * sessionMultipliers[sessionIndex];
      ...
      final double upperWickSize = minWickSize + (maxWickSize - minWickSize) * _random.nextDouble();
      final double lowerWickSize = minWickSize + (maxWickSize - minWickSize) * _random.nextDouble();
  ...
  static double _generateMomentum(int index, List<Map<String, double>> previousCandles) {
    ...
    final double momentumStrength = (totalMovement / 3).abs() * 0.3; // 30% of recent movement
    final double momentumDirection = totalMovement > 0 ? 1.0 : -1.0;
    
    return momentumStrength * momentumDirection * (1.0 - (index % 10) / 10.0); // Fade over time
  }
