...
class SypProvider extends GetxController {
  ...
  void _log(String message, {String? method, dynamic data, String level = 'info'}) {
  ...
  Future<void> loadCurrentRates({bool forceRefresh = false}) async {
    ...
    try {
      ...
      if (_currentRates != null) {
        _log('💱 Rate: ${_currentRates!.currentRates.mid}', method: method, level: 'debug');
        _log('🏙️ City: ${_currentRates!.city}', method: method, level: 'debug');
        _log('📊 Day type: ${_currentRates!.ohlcv.dayType}', method: method, level: 'debug');
      }
      
      update();
      _log('✅ Listeners notified of current rates update', method: method, level: 'debug');
  ...
  Future<void> loadForecast({bool forceRefresh = false}) async {
    ...
    try {
      ...
      // Log data models
      if (_forecast != null) {
        _log('🔮 Predicted rate: ${_forecast!.prediction.rate}', method: method, level: 'debug');
        _log('📅 Forecast date: ${_forecast!.forecastDate}', method: method, level: 'debug');
        _log('📊 Day type: ${_forecast!.prediction.dayType}', method: method, level: 'debug');
      }
      
      update();
      _log('✅ Listeners notified of forecast update', method: method, level: 'debug');
  ...
  Future<void> refreshData() async {
    ...
    try {
      ...
      _log('✅ Full data refresh completed successfully', method: method, level: 'info');
      _log('⏱️ Total refresh time: ${duration.inMilliseconds}ms', method: method, level: 'debug');
  ...
  void _setLoading(bool loading) {
    _log('⏳ Setting loading state: $loading (was: $_isLoading)', level: 'debug');
    _isLoading = loading;
    update();
  ...
  bool isPositiveChange(double change) {
    ...
    _log('🎨 Change color check: $change -> ${isPositive ? "positive" : "negative"}', level: 'debug');
    return isPositive;
  }

  bool isCalmDay(String dayType) {
    final isCalm = dayType.toLowerCase() == 'calm';
    _log('🌤️ Day type check: $dayType -> ${isCalm ? "calm" : "normal"}', level: 'debug');
    return isCalm;
  }
  ...
  Future<bool> testServerConnection() async {
    ...
    try {
      ...
      _log('✅ Server connection test successful', method: method, level: 'info');
      return true;
    } catch (e) {
  ...
  void debugCurrentState() {
    ...
    _log('💱 Current rates: ${_currentRates != null ? "loaded" : "null"}', method: method, level: 'debug');
    _log('🔮 Forecast: ${_forecast != null ? "loaded" : "null"}', method: method, level: 'debug');
  }
}